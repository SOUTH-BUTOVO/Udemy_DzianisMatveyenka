...
/**     1 Создать класс Hero, представляющий собой героя и содержащий поле name.
    ...
    Переопределить в них метод attackEnemy() для вывода специализированного для этого класса сообщение об атаке.
    ...
    Добавит метод takeDamage(int damage), который уменьшает кол-во здоровья в соответствии с полученным уроном.
    Переписать метод attackEnemy() класса Hero, добавив ему параметр типа Enemy.
    Метод должен вызвать у врага, метод takeDamage и передавать в него определённое количество урона.
    Перео-ть метод в подклассах Warrior, Mage и Archer, что бы каждый герой наносил врагу разное кол-во урона.
        4 Сделать класс Hero и его метод attackEnemy() абстрактным.
        5 Создать интерфейс Mortal, содержащий метод isAlive().
    Сделать так, что бы класс Enemy реализовал интерфейс Mortal.
    Определить метод isAlive() в классе Enemy, что бы тот возвращал true, если кол-во здоровья врага больше 0.
    ...
    Создать несколько видов врагов (наследников класса Enemy) с разными способностями (например, Zombie - имеет
    шанс воскреснуть при гибели).
    Дать героям уникальные способности.
    Продемонстрировать сражение героя с несколькими соперниками. */

public class TrainingGround {
    public static void main(String[] args) {
        Hero warrior = new Warrior("Арагорн", 15);
        Hero mage = new Mage("Гэндальф", 20);
        Hero archer = new Archer("Леголас", 10);

        Enemy enemy = new Enemy("Саурон", 100);

        attackEnemy(enemy, warrior, mage, archer);

    }

    static void attackEnemy(Enemy enemy, Hero... heroes) {
        while (enemy.isAlive()) {
